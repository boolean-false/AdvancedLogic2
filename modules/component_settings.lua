...
function M.specs(name)
 name=base_name(name)
 if name=='advanced_logic:pulse_generator' then
  return {options('pulse_duration','Длительность, секунды',{0.05,0.1,0.25,0.5,1})}
 end
 if name=='advanced_logic:bus_splitter_4bit' then
  return {options('group','Диапазон битов',{0,1,2,3},{'0..3','4..7','8..11','12..15'})}
 end
 if name=='advanced_logic:bus_slice' or name=='advanced_logic:bus_join' then
  return {options('part_bits','Ширина каждой части',{4,8},{'4 + 4','8 + 8'})}
 end
 local width=options('data_bits','Ширина данных',{1,4,8,16})
 if name=='advanced_logic:bus_bit_reader_4bit' or name=='advanced_logic:bus_bit_writer_4bit' then
  local bits={};for i=0,15 do bits[#bits+1]=i end
  local index=options('selected_bit','Номер бита (0 - младший)',bits)
  if name=='advanced_logic:bus_bit_reader_4bit' then return {index} end
  return {width,index}
 end
 if name=='advanced_logic:bus_logic' then
  return {width,options('operation','Операция',{0,1,2,3},{'AND','OR','XOR','NOT'})}
 end
 return {width}
end
function M.open(x,y,z,playerid)
 if not require('advanced_logic:configurator_check').can_open_ui(playerid) then return false end
 ...
 hud.show_overlay('advanced_logic:component_settings',false)
 return true
end
