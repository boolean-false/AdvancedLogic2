// Hand-designed board: explicit component positions and wire polylines.
// No placement search or automatic routing. Run with Node from any directory.
...
// Datapath: accumulator feedback, immediate operand, three ALU results.
...
// Program address and opcode. The MUX consumes the low two opcode bits.
...
// RAM address selection and readback, independent of its write data.
wire('address',A,[32,11],[32,6]);wire('keypad',A,[30,14],[30,12],[31,12]);
wire('inspect',red,[36,12],[33,12]);wire('ram',B,[32,4],[32,2]);
// One physical clock network for all sequential components.
...
// Terminal directions are explicit. Keep a real conductor at every device
// contact: passive bridges cannot attach directly to component ports.
const terminals={
...
// At parallel adjacent tracks, a straight bridge isolates side contacts. A turn
// must never be converted: it would disconnect the intended net.
...
// A labelled plan for the manual, using the same fixed board coordinates.
...
svg+='<text x="0" y="-1.8" font-family="sans-serif" font-size=".85" fill="#fff">8-bit machine · top view · CLOCK = placement origin</text></svg>\n';
