...
local function update_width_labels()
    for _, n in ipairs({ 1, 4, 8, 16 }) do document['width_' .. n].text = (n == selected_width and '[x] ' or '') ..
        n .. ' бит' end
...
function apply_constant()
    if not ui.valid(bx, by, bz, 'advanced_logic:bus_constant') then
        close_constant(); return
    end
    local input = document["value_input"]
    local value, err = ui.number(input and input.text, 0, bus.mask(selected_width), true)
    if not value then
        document.feedback.text = err; return
    end
